# Three-Minute Demo Script

> Status: Final video published and verified · Date: 2026-08-17 · Owner: Presenter

## Recording rules

- Keep the final video under three minutes; judges are not required to watch beyond three minutes.
- Show the functioning project, not slides alone.
- Keep `SYNTHETIC DEMO` and the real provider/fallback state visible.
- It is safe to show the verified public Cloud Run/Vertex demo. Do not say customer production, revenue, real users, or measured impact.
- Use only licensed/owned visuals and audio. The public upload gate was approved and completed.

## Verified render

- Local file: `videos/edit/final.mp4`
- Duration: 170 seconds
- Video: 1920×1080, 30 fps, H.264
- Audio: AAC
- Captions: 74
- Publication: [youtu.be/QOPgHHAWOBA](https://youtu.be/QOPgHHAWOBA), verified player 2:50, published 2026-08-17
- Custom thumbnail: not set because YouTube phone verification is required; do not claim one

## 0:00–0:20 — Problem and disclosure

**On screen:** DeployAlign title, `SYNTHETIC DEMO`, gate `HOLD`, provider badge.

**Say:**

“Deployment risk often begins as document drift: a customer asks for an outcome, sales turns it into a broad promise, and engineering evidence is narrower. DeployAlign compiles those artifacts into a source-linked decision. This is a synthetic demonstration, not a customer deployment.”

## 0:20–0:50 — Three sources

**On screen:** Customer, sales, and engineering artifact cards; highlight the exact conflicting phrases.

**Say:**

“Our fictional customer and sales artifacts ask for all materials, every area, and full autonomy. The engineering artifact supports five named analytes, 12 mapped AOIs, supervised operation, and still requires a blind test and physical survey.”

## 0:50–1:25 — Compile and diagnose

**Action:** Run compile or reveal the compiled state.

**On screen:** Typed graph, six diagnostic codes, exact source links, unresolved blocker count.

**Say:**

“This public Cloud Run demo calls Gemini 2.5 Flash through Vertex AI. Gemini returns three schema-validated candidates grounded in exact source quotes. Deterministic TypeScript then builds the canonical graph and emits stable diagnostics: unbounded scope, unsupported commitments, a preference promoted to a constraint, an unverified site claim, missing acceptance criteria, and an open critical test. The provider badge and success receipt make the live path visible.”

## 1:25–1:55 — Minimum semantic patch

**On screen:** Before/after patch rows.

**Say:**

“DeployAlign does not invent pricing, timing, measurements, or a success threshold. It proposes only three bounded changes supported by the synthetic evidence: five named analytes, 12 mapped AOIs, and supervised Phase 1.”

## 1:55–2:25 — Human review and remaining gates

**Action:** Trigger the local demo review.

**On screen:** Version changes from 1 to 2; gate becomes `CONDITIONAL PILOT`; one blocker remains open and DA-004/DA-006 still require evidence.

**Say:**

“A human review action advances the local demo baseline, but not to an unconditional pass. The blind test and site survey remain open. This button demonstrates a review boundary; it is not authenticated approval for a real deployment.”

## 2:25–2:48 — Incremental outputs and receipts

**On screen:** Customer memo, SOW, test manifest, impact list, stable FNV-1a32 change fingerprints, execution receipts.

**Say:**

“One decision ID propagates into three audiences. Six linked sections rebuild, while three unrelated canonical sections from this compile's fresh baseline remain unchanged. Their FNV-1a32 fingerprints show change only; they are not security proofs. Receipts identify Gemini extraction, deterministic rules, review, and build stages.”

## 2:48–3:00 — Honest close

**Say:**

“DeployAlign’s thesis is simple: compile promises into bounded, evidence-linked decisions before they become field risk. The Cloud Run and Gemini path is live for this synthetic demo. Real user validation and business evidence are still next; neither is claimed today.”

## Evidence to capture during recording

- Provider and synthetic badges.
- `gemini-2.5-flash` success receipt and three grounded `AI_DRAFT` classifications.
- Exact source quote navigation.
- Pre/post gate and version.
- Six diagnostics and two remaining open items.
- Six Decision-ID-linked sections rebuilt and three unrelated canonical sections reused.
- No key, token, personal data, account identifier, or private financial information.
